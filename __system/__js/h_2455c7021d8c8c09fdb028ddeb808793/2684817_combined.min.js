...
;(function(factory) {
    ...
    Slick.prototype.checkResponsive = function(initial, forceUpdate) {
        ...
        // Hack to get demo mode width working
        var ec_width      = $('#editor_content').css("width");
        var ec_width_int  = parseInt(ec_width);
 
        if(windowWidth > ec_width_int){
          windowWidth = ec_width_int;
        }
    ...
    Slick.prototype.resize = function() {
        ...
        var ec_width      = $('#editor_content').css("width");
        var ec_width_int  = parseInt(ec_width);
        if(isNaN(ec_width_int)) ec_width_int = 0;
 ...
 * Date: 2014-10-13
 */
(function(b,a,c){b.fn.captionjs=function(e){var g={class_name:"captionjs",schema:true,mode:"default",debug_mode:false,force_dimensions:true,is_responsive:false,inherit_styles:false};var d=b.extend(g,e||{});var f=function(k,i,j,h){if(j.jquery&&h.jquery){j.css(k,h.css(k));h.css(k,i)}};return this.each(function(){if(d.debug_mode){console.log("caption.js | Starting.")}var n=b(this),o=n.data("caption")?n.data("caption"):n.attr("alt"),j=n.wrap('<figure class="'+d.class_name+" "+d.mode+'"/>').after("<figcaption/>").parent(),m=n.next("figcaption").html(o),i=n.data("link")?m.wrapInner('<a href="'+n.data("link")+'"/>').children("a").css("padding","0").css("margin","0"):null,h,k;if(o===""){m.remove()}if(d.debug_mode){console.log("caption.js | Caption: "+o)}if(d.force_dimensions){if(d.debug_mode){console.log("caption.js | Forcing dimensions with a clone.")}var l=j.clone().css({position:"absolute",left:"-9999px"}).appendTo("body");h=b("img",l).outerWidth();k=b("figcaption",l).css("width",h).css("clear","both").outerHeight();l.remove()}else{h=n.outerWidth();k=m.outerHeight()}if(d.is_responsive){h="100%";n.width(h)}if(d.inherit_styles){if(n.css("display")=="inline"){j.css("display","inline-block")}else{f("display","block",j,n)}if(n.css("position")=="static"){j.css("position","relative")}else{f("position","relative",j,n)}f("clear","both",j,n);f("float","none",j,n);f("margin","0",j,n);n.css("padding","0");f("left","auto",j,n);f("right","auto",j,n);f("top","auto",j,n);f("bottom","auto",j,n);f("z-index",n.css("z-index"),j,n)}j.width(h);if(d.schema){j.attr({itemscope:"itemscope",itemtype:"http://schema.org/Photograph"});m.attr("itemprop","name");n.attr("itemprop","image")}if(d.mode==="stacked"){m.css({"margin-bottom":"0",bottom:"0",})}if(d.mode==="animated"){m.css({"margin-bottom":"0",bottom:-k,})}if(d.mode==="hide"){m.css({"margin-bottom":k,bottom:-k,})}})}})(jQuery,window);;
...
(function($) {
	...
	$.fn.lightBox = function(settings) {
      ...
      imageLoading:     '/static/images/global/lightbox/lightbox-ico-loading.gif',    // (string) Path and the name of the loading icon
      imageBtnPrev:     '/static/images/global/lightbox/lightbox-btn-prev.gif',     // (string) Path and the name of the prev button image
      imageBtnNext:     '/static/images/global/lightbox/lightbox-btn-next.gif',     // (string) Path and the name of the next button image
      imageBtnClose:      '/static/images/global/lightbox/lightbox-btn-close.gif',    // (string) Path and the name of the close btn
      imageBlank:       '/static/images/global/lightbox/lightbox-blank.gif',      // (string) Path and the name of a blank image (one pixel)
			...
			imageArray:				[],
			activeImage:			0,
      showDescription:  2,   // 1 == YES, 2 == NO
      parent_div_id: null
		},settings);
		// Caching the jQuery object with all elements matched
		var jQueryMatchedObj = this; // This, in this context, refer to jQuery object
    var $parent_div      = $('#' + settings.parent_div_id).parent();
		...
		function _set_interface() {
			...
			$('body').append('<div id="jquery-overlay" class="jquery-overlay"></div><div id="jquery-lightbox"><div id="lightbox-container-image-box"><div id="lightbox-container-image"><img id="lightbox-image"><div style="" id="lightbox-nav"><a href="#" id="lightbox-nav-btnPrev"></a><a href="#" id="lightbox-nav-btnNext"></a></div><div id="lightbox-loading"><a href="#" id="lightbox-loading-link"><img src="' + settings.imageLoading + '"></a></div></div></div><div id="lightbox-container-image-data-box"><div id="lightbox-container-image-data"><div id="lightbox-image-details"><span id="lightbox-image-details-caption"></span></div><div id="lightbox-secNav"><span id="lightbox-image-details-currentNumber"></span><a href="#" id="lightbox-secNav-btnClose"><img src="' + settings.imageBtnClose + '"></a></div></div></div></div>');	
			// Get page sizes
		...
		function _show_image() {
			...
			$('#lightbox-image').fadeIn(function() {
				_show_image_data();
				_set_navigation();
        $('#lightbox-image-details-currentNumber').fadeIn();
			});
        ...
        if(settings.showDescription == 1){
          ...
          var $description_container = $('#lightbox-image-details-caption');
          ...
          if(d_white_space) { $description_container.css('white-space', d_white_space);}
		      ...
		      $description_container.html(settings.imageArray[settings.activeImage][1]).show();
        }
        else{
		      $('#lightbox-image-details-caption').html('&nbsp;').show();
        }
		  }

			// If we have a image set, display 'Image X of X'
			if ( settings.imageArray.length > 1 ) {
				$('#lightbox-image-details-currentNumber').html(settings.txtImage + ' ' + ( settings.activeImage + 1 ) + ' ' + settings.txtOf + ' ' + settings.imageArray.length);
			}		
...
(function($) { $("#35d4bbc6c295dd35ea0412ad067ddf1e").iv_widget({ values: "D%2BZHgqRpeym2KNzR9RsaOY4JFnrjva3hvBsZBNNYvByJi8o7IHm8TIThphUg0H2bDZmK8y4SyzzGMhiYtAh7tv%2BJoPUW3%2BjaidLoDr4Uw3ZkJhAFgyS9%2BqWWKvDRurV73xB8qrCQ2whW75c04rMk6DsYznoH02RVfwC83BXeIatRAvnjgHQAYE4FVH7skz8Un7z2g4SyPRphov1DUEeZaeukFPLL2jPbv0lUe2HhtyVCBarFpjR%2B4Oaj%2FpgEJQDcoEA7Aw4Sh2P5rVYo2FIBFVIc0r3eUPsyJA7S7up6SX%2FO64k1TTvXJg8YFe2trus9oA1oh0gyFVAqG3%2F%2Bx6%2BFGdFpN2TxCepgiekL35bvk7CG0w6Fc494taYF%2F8vCHwGFp2q4YyemdcEjUkpuzg%3D%3D", no_parent: false }); })(jQuery);

      jQuery("#slick_index_27547").slick({      galleryType: 'grid',      lazyLoad: 'ondemand',      dots: true,      infinite: true,      rows: 1,      slidesPerRow: 4,      slidesToScroll: 1,      autoplay: false,      autoplaySpeed: 5000,      centerMode: false,      respondsTo: 'slider',      centerPadding: '0px',responsive: [{        breakpoint: 1001,        settings: {                      galleryType: 'grid',                      arrows: true,                      centerMode: false,                      centerPadding: '0px',                      rows: 2,                      slidesPerRow: 2,                      slidesToScroll: 1,                      infinite: true,                      dots: true                    }},{        breakpoint: 771,        settings: {                      galleryType: 'grid',                      arrows: true,                      centerMode: false,                      centerPadding: '0px',                      rows: 2,                      slidesPerRow: 1,                      slidesToScroll: 1,                      infinite: true,                      dots: true                    }},{        breakpoint: 501,        settings: {                      galleryType: 'grid',                      arrows: true,                      centerMode: false,                      centerPadding: '0px',                      rows: 1,                      slidesPerRow: 1,                      slidesToScroll: 1,                      infinite: true,                      dots: false                    }}]});

jQuery("#slick_index_27547").slick('slickPlay'); 

jQuery("#slick_index_27547 img").each(function(){      jQuery(this).captionjs({ mode : "animated" });    }); 
jQuery("#slick_index_27547 figure").each(function(){      jQuery(this).css("width","100%");    }); 
...
jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk2_blk4").ivfx_fade_in({"class":"ivfx_fade_in","breakpoints":[0,1000,770,499],"breakpoint_params":{"770":{"params":{"__bring_to_front":"0","__trigger_event":"top","__duration":"2","__direction":"up"},"width":"770","is_enabled":1,"name":"phablet"},"499":{"name":"phone","params":{"__bring_to_front":"0","__trigger_event":"top","__duration":"2","__direction":"up"},"is_enabled":1,"width":"499"},"1000":{"width":"1000","is_enabled":1,"params":{"__bring_to_front":"0","__trigger_event":"top","__duration":"2","__direction":"up"},"name":"tablet"},"0":{"name":"desktop","params":{"__duration":"2","__direction":"up","__bring_to_front":"0","__trigger_event":"top"},"is_enabled":1,"width":"0"}}})


jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk2_blk1").ivfx_fade_in({"class":"ivfx_fade_in","breakpoints":[0,1000,770,499],"breakpoint_params":{"1000":{"params":{"__direction":"up","__duration":"0.5","__trigger_event":"top","__bring_to_front":"0"},"width":"1000","is_enabled":1,"name":"tablet"},"770":{"name":"phablet","params":{"__trigger_event":"top","__bring_to_front":"0","__direction":"up","__duration":"0.5"},"width":"770","is_enabled":1},"499":{"name":"phone","width":"499","is_enabled":1,"params":{"__trigger_event":"top","__bring_to_front":"0","__direction":"up","__duration":"0.5"}},"0":{"params":{"__direction":"up","__duration":"0.5","__trigger_event":"top","__bring_to_front":"0"},"is_enabled":1,"width":"0","name":"desktop"}}})

jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk4_blk1_blk2").ivfx_fade_in({"class":"ivfx_fade_in","breakpoints":[0,1000,770,499],"breakpoint_params":{"0":{"width":"0","is_enabled":1,"params":{"__bring_to_front":"0"},"name":"desktop"},"499":{"name":"phone","is_enabled":1,"width":"499","params":{"__bring_to_front":"0"}},"770":{"name":"phablet","is_enabled":1,"width":"770","params":{"__bring_to_front":"0"}},"1000":{"width":"1000","is_enabled":1,"params":{"__bring_to_front":"0"},"name":"tablet"}}})

jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk3_blk1_blk3").ivfx_fade_in({"class":"ivfx_fade_in","breakpoints":[0,1000,770,499],"breakpoint_params":{"0":{"name":"desktop","params":{"__bring_to_front":"0","__trigger_event":"top","__direction":"right"},"width":"0","is_enabled":1},"770":{"name":"phablet","is_enabled":1,"width":"770","params":{"__trigger_event":"top","__bring_to_front":"0","__direction":"right"}},"499":{"is_enabled":1,"width":"499","params":{"__bring_to_front":"0","__trigger_event":"top","__direction":"right"},"name":"phone"},"1000":{"width":"1000","is_enabled":1,"params":{"__direction":"right","__bring_to_front":"0","__trigger_event":"top"},"name":"tablet"}}})

jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk3_blk1_blk2").ivfx_fade_in({"class":"ivfx_fade_in","breakpoints":[0,1000,770,499],"breakpoint_params":{"0":{"name":"desktop","params":{"__direction":"right","__bring_to_front":"0","__trigger_event":"top"},"is_enabled":1,"width":"0"},"499":{"name":"phone","params":{"__bring_to_front":"0","__trigger_event":"top","__direction":"right"},"is_enabled":1,"width":"499"},"770":{"name":"phablet","is_enabled":1,"width":"770","params":{"__bring_to_front":"0","__trigger_event":"top","__direction":"right"}},"1000":{"name":"tablet","width":"1000","is_enabled":1,"params":{"__direction":"right","__bring_to_front":"0","__trigger_event":"top"}}}})

jQuery("#rot_ctr1_bod_ctr2_bod_wrp1").ivfx_background_parallax({"breakpoint_params":{"0":{"name":"desktop","width":"0","is_enabled":1,"params":{}},"770":{"name":"phablet","width":"770","is_enabled":1,"params":{}},"499":{"is_enabled":1,"width":"499","params":{},"name":"phone"},"1000":{"params":{},"is_enabled":1,"width":"1000","name":"tablet"}},"breakpoints":[0,1000,770,499],"class":"ivfx_background_parallax"})

jQuery("#rot_ctr1_bod_ctr1").ivfx_pin({"breakpoints":[0,1000,770,499],"breakpoint_params":{"0":{"name":"desktop","params":{},"width":"0","is_enabled":1},"499":{"name":"phone","is_enabled":1,"width":"499","params":{}},"770":{"name":"phablet","params":{},"is_enabled":1,"width":"770"},"1000":{"name":"tablet","params":{},"width":"1000","is_enabled":1}},"class":"ivfx_pin"})

jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk2_blk2").ivfx_fade_in({"breakpoint_params":{"770":{"is_enabled":1,"width":"770","params":{"__direction":"up","__trigger_event":"top","__bring_to_front":"0"},"name":"phablet"},"499":{"name":"phone","params":{"__bring_to_front":"0","__trigger_event":"top","__direction":"up"},"is_enabled":1,"width":"499"},"1000":{"params":{"__trigger_event":"top","__bring_to_front":"0","__direction":"up"},"is_enabled":1,"width":"1000","name":"tablet"},"0":{"is_enabled":1,"width":"0","params":{"__trigger_event":"top","__bring_to_front":"0","__direction":"up"},"name":"desktop"}},"breakpoints":[0,1000,770,499],"class":"ivfx_fade_in"})

jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk2_blk3").ivfx_fade_in({"class":"ivfx_fade_in","breakpoints":[0,1000,770,499],"breakpoint_params":{"1000":{"params":{"__duration":"1.5","__direction":"up","__bring_to_front":"0","__trigger_event":"top"},"width":"1000","is_enabled":1,"name":"tablet"},"770":{"name":"phablet","params":{"__duration":"1.5","__direction":"up","__bring_to_front":"0","__trigger_event":"top"},"width":"770","is_enabled":1},"499":{"name":"phone","width":"499","is_enabled":1,"params":{"__bring_to_front":"0","__trigger_event":"top","__duration":"1.5","__direction":"up"}},"0":{"name":"desktop","is_enabled":1,"width":"0","params":{"__direction":"up","__duration":"1.5","__trigger_event":"top","__bring_to_front":"0"}}}})

jQuery("#rot_ctr1_bod_ctr2_bod_wrp1_blk3_blk1_blk1").ivfx_fade_in({"class":"ivfx_fade_in","breakpoint_params":{"1000":{"width":"1000","is_enabled":1,"params":{"__trigger_event":"bottom","__bring_to_front":"0"},"name":"tablet"},"499":{"name":"phone","params":{"__bring_to_front":"0","__trigger_event":"bottom"},"is_enabled":1,"width":"499"},"770":{"name":"phablet","params":{"__bring_to_front":"0","__trigger_event":"bottom"},"is_enabled":1,"width":"770"},"0":{"name":"desktop","is_enabled":1,"width":"0","params":{"__trigger_event":"bottom","__bring_to_front":"0"}}},"breakpoints":[0,1000,770,499]})
